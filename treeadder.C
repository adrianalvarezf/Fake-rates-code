...
void treeadder(TString dir ,TString channel, int bins){
  ...
  for(int j=0;j<8;j++){
    ...
    TFile *f_all = new TFile(dir+"/"+Form("files_%dbins",bins)+"/"+file,"recreate");
    TFile *f[bins-1];
    f[0]=new TFile(dir+"/"+ subdir +"_bvetobtag/"+file);//"_exclusive/"+file);
    f[1]=new TFile(dir+"/"+ subdir +"_loosebtag/"+file);//"_exclusive/"+file);
    if(bins==4){
      f[2]=new TFile(dir+"/"+ subdir +"_mediumbtag/"+file);//"_exclusive/"+file);
    f[3]=new TFile(dir+"/"+ subdir +"_tightbtag/"+file);//"_exclusive/"+file);
    }
    else if(bins==3) f[2]=new TFile(dir+"/"+ subdir +"_mediumtightbtag/"+file);//"_exclusive/"+file);
